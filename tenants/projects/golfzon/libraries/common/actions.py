#
#  actions.py - GolfzonCommon Library Actions
#

import mcp.types as types
import os
from tenants.base import BaseTenant
from tenants.projects.golfzon.libraries.common.axioms import Axioms
from tenants.projects.golfzon.libraries.common.overview import Overview
from tenants.projects.golfzon.libraries.common.blueprint import BluePrint
from core.logger import HubLogger
from language.swift.auditor import SwiftAuditor

class GolfzonCommonTenant(BaseTenant):
    def __init__(self):
        self.logger = HubLogger("GolfzonCommon")
        self.auditor = SwiftAuditor(logger=self.logger)

    def get_name(self) -> str:
        return "GolfzonCommon"

    def get_tools(self) -> list[types.Tool]:
        return [
            types.Tool(
                name="common_audit_rules",
                description="GolfzonCommon 라이브러리 컨벤션 준수 여부를 진단합니다.",
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            types.Tool(
                name="common_get_overview",
                description="GolfzonCommon 라이브러리의 요약 정보(경로, 도메인)를 확인합니다.",
                inputSchema={"type": "object", "properties": {}},
            ),
            types.Tool(
                name="common_get_blueprint",
                description="GolfzonCommon 라이브러리의 상세 설계도를 확인합니다.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "domain": {"type": "string", "description": "상세 설계도를 볼 도메인 이름 (예: core/config, system/services)"}
                    }
                },
            )
        ]
    ...
    async def call_tool(self, name: str, arguments: dict) -> list[types.TextContent]:
        self.logger.log(f"GolfzonCommon 도구 호출: {name}", 0)
        
        func_name = name.replace("common_", "")
        
        if func_name == "get_overview":
            briefing = Overview.get_briefing()
            res = (
                f"📦 [GolfzonCommon Overview]\n"
                f"---\n"
            ...
            if domain:
                ...
                return [types.TextContent(type="text", text=f"📐 [GolfzonCommon Master]\n---\n🔄 Flow: {master['flow']}\n🎯 Purpose: {master['purpose']}")]
            ...
            # 라이브러리 전용 추가 감사 로직 (예: public인데 주석 없는 경우 등)
            if "public" in content and "///" not in content:
                results.append("⚠️ [Docs] public 인터페이스에는 문서화 주석(///)이 권장됩니다.")

            if not results:
                status_msg = "✅ PASS: 모든 공리를 준수하고 있습니다! 🚀"
                return [types.TextContent(type="text", text=f"🔍 [Common Audit] {os.path.basename(file_path)}\n---\n{status_msg}")]
            else:
                violations = "\n".join(results)
                error_res = (
                    ...
                    f"위반 내역:\n{violations}\n\n"
                    f"⚠️ 주의: 라이브러리 컨벤션을 준수하지 않으면 배포가 제한될 수 있습니다. 🛡️"
                )
