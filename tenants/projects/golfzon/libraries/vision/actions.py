#
#  actions.py - GolfzonVision Library Actions
#

import mcp.types as types
import os
from tenants.base import BaseTenant
from tenants.projects.golfzon.libraries.vision.axioms import Axioms
from tenants.projects.golfzon.libraries.vision.overview import Overview
from tenants.projects.golfzon.libraries.vision.blueprint import BluePrint
from core.logger import HubLogger
from language.swift.auditor import SwiftAuditor

class GolfzonVisionTenant(BaseTenant):
    def __init__(self):
        self.logger = HubLogger("GolfzonVision")
        self.auditor = SwiftAuditor(logger=self.logger)

    def get_name(self) -> str:
        return "GolfzonVision"

    def get_tools(self) -> list[types.Tool]:
        return [
            types.Tool(
                name="vision_audit_rules",
                description="GolfzonVision 라이브러리 컨벤션 준수 여부를 진단합니다.",
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            types.Tool(
                name="vision_get_overview",
                description="GolfzonVision 라이브러리의 요약 정보(경로, 도메인)를 확인합니다.",
                inputSchema={"type": "object", "properties": {}},
            ),
            types.Tool(
                name="vision_get_blueprint",
                description="GolfzonVision 라이브러리의 상세 설계도를 확인합니다.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "domain": {"type": "string", "description": "상세 설계도를 볼 도메인 이름 (예: hybrid/web, core/prime)"}
                    }
                },
            )
        ]
    ...
    async def call_tool(self, name: str, arguments: dict) -> list[types.TextContent]:
        self.logger.log(f"GolfzonVision 도구 호출: {name}", 0)
        
        func_name = name.replace("vision_", "")
        
        if func_name == "get_overview":
            briefing = Overview.get_briefing()
            res = (
                f"👁️ [GolfzonVision Overview]\n"
                f"---\n"
            ...
            if domain:
                ...
                return [types.TextContent(type="text", text=f"📐 [GolfzonVision Master]\n---\n🔄 Flow: {master['flow']}\n🎯 Purpose: {master['purpose']}")]
            ...
            if not results:
                ...
                return [types.TextContent(type="text", text=f"🔍 [Vision Audit] {os.path.basename(file_path)}\n---\n{status_msg}")]
            else:
                violations = "\n".join(results)
                error_res = (
                    ...
                    f"위반 내역:\n{violations}\n\n"
                    f"⚠️ 주의: Vision 모듈은 서비스의 얼굴입니다. 공리를 철저히 준수하여 무결성을 유지하십시오. 🛡️"
                )
