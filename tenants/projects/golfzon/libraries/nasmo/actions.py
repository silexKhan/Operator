#
#  actions.py - GolfzonNasmo Library Actions
#

import mcp.types as types
import os
from tenants.base import BaseTenant
from tenants.projects.golfzon.libraries.nasmo.axioms import Axioms
from tenants.projects.golfzon.libraries.nasmo.overview import Overview
from tenants.projects.golfzon.libraries.nasmo.blueprint import BluePrint
from core.logger import HubLogger
from language.swift.auditor import SwiftAuditor

class GolfzonNasmoTenant(BaseTenant):
    def __init__(self):
        self.logger = HubLogger("GolfzonNasmo")
    ...
    def get_tools(self) -> list[types.Tool]:
        return [
            types.Tool(
                name="nasmo_audit_rules",
                description="GolfzonNasmo 라이브러리 컨벤션 준수 여부를 진단합니다.",
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            types.Tool(
                name="nasmo_get_overview",
                description="GolfzonNasmo 라이브러리의 요약 정보(경로, 도메인)를 확인합니다.",
                inputSchema={"type": "object", "properties": {}},
            ),
            types.Tool(
                name="nasmo_get_blueprint",
                description="GolfzonNasmo 라이브러리의 상세 설계도를 확인합니다.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "domain": {"type": "string", "description": "상세 설계도를 볼 도메인 이름 (예: player/dual, drawing/tools)"}
                    }
                },
            )
        ]
    ...
    async def call_tool(self, name: str, arguments: dict) -> list[types.TextContent]:
        self.logger.log(f"GolfzonNasmo 도구 호출: {name}", 0)
        
        func_name = name.replace("nasmo_", "")
        
        if func_name == "get_overview":
            briefing = Overview.get_briefing()
            res = (
                f"🎥 [GolfzonNasmo Overview]\n"
                f"---\n"
            ...
            if domain:
                ...
                return [types.TextContent(type="text", text=f"📐 [GolfzonNasmo Master]\n---\n🔄 Flow: {master['flow']}\n🎯 Purpose: {master['purpose']}")]
            ...
            if not results:
                ...
                return [types.TextContent(type="text", text=f"🔍 [Nasmo Audit] {os.path.basename(file_path)}\n---\n{status_msg}")]
            else:
                violations = "\n".join(results)
                error_res = (
                    ...
                    f"위반 내역:\n{violations}\n\n"
                    f"⚠️ 주의: 나스모 플레이어 관련 코드는 복잡도가 높으므로 공리를 철저히 준수해야 합니다. 🛡️"
                )
