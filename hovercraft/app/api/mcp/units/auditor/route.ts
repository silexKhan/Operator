...
/**
 * [사용자] 특정 전문 유닛(Unit)의 감사기(Auditor.py) 소스 코드를 읽고 수정합니다. 
 */
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const auditorPath = process.env.MCP_ROOT ? path.join(process.env.MCP_ROOT, 'units', unitName.toLowerCase(), 'auditor.py') : path.join(process.cwd(), '..', 'units', unitName.toLowerCase(), 'auditor.py');
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const auditorPath = process.env.MCP_ROOT ? path.join(process.env.MCP_ROOT, 'units', name.toLowerCase(), 'auditor.py') : path.join(process.cwd(), '..', 'units', name.toLowerCase(), 'auditor.py');
