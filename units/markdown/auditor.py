...
class MarkdownAuditor(BaseAuditor):
    ...
    def _extract_text_from_pptx(self, file_path: str) -> str:
        ...
        try:
            ...
            self.log(f"⚠️ PPTX 추출 실패: {str(e)}", 2)
            return ""
    ...
    def audit(self, file_path: str, content: str = "") -> list[str]:
        ...
        # 예: '유저' 대신 '사용자' 권장
        if "유저" in content:
            results.append("⚠️ WARNING: Protocol (Terminology) - '유저' 대신 '사용자'라는 용어를 사용하십시오. 🗣️")

        # 3. Global Protocol 0-1 (Mechanical Integrity) 체크
        if "..." in content or "중략" in content:
            results.append("❌ FAIL: Protocol 0-1 (Global) - 문서 내에 '...' 또는 '중략'을 사용하지 마십시오. 완전한 명세가 필요합니다. 🛠️🛑")
