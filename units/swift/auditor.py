...
class SwiftAuditor(BaseAuditor):
    ...
    def _get_protocols_for_path(self, file_path: str):
        """
        파일 경로를 기반으로 회선(Circuit)이 정의한 프로토콜을 동적으로 가져옵니다. 🛡️
        """
        if self.circuit_manager:
            # 활성화된 회선에서 프로토콜 추출
            active = self.circuit_manager.get_active_circuit()
            if active and active.get_name() == "GDR":
    ...
    def audit(self, file_path: str, content: str) -> list[str]:
        ...
        if re.search(subject_pattern, content):
            results.append("❌ FAIL: Protocol S-5 (Combine Naming) - 'Subject' 접미사가 포함된 변수가 발견되었습니다. ⛓️")
        ...
        if is_view_controller:
            ...
            if logic_keywords > 15:
                results.append(f"⚠️ WARNING: Protocol S-1 (Strict MVVM) - {logic_keywords}개의 조건문이 발견되었습니다. Dumb View 원칙을 확인하십시오.")
            
            if "!" in content and "IBOutlet" not in content:
                results.append("❌ FAIL: Protocol S-4 (No Forced Unwrapping) - 강제 언래핑('!') 사용이 발견되었습니다. 🛡️")
        ...
        if "..." in content or "중략" in content:
            results.append("❌ FAIL: Protocol 0-1 (Mechanical Integrity) - 코드 내에 '...' 또는 '중략'을 사용하지 마십시오. 완전한 리터럴 텍스트가 필요합니다. 🛠️🛑")
