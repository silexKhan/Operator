...
import re
import os
from language.python.axioms import PythonAxioms
...
class PythonAuditor:
    ...
    def audit(self, file_path: str, content: str) -> list[str]:
        results = []
        self.log(f"Python 감사 시작 - 파일: {os.path.basename(file_path)}", 1)

        # 1. Strict Type Hinting (Axiom P1)
        # 함수 정의에서 -> 가 없는 경우 찾기
        missing_hints = re.findall(r"def\s+\w+\(.*\)(?!\s*->)", content)
        if missing_hints:
            self.log("Type Hinting 위반 발견", 2)
            results.append("❌ FAIL: Axiom P1 (Strict Type Hinting) - 함수 반환 타입을 '->'를 통해 명시하십시오.")

        # 2. Pydantic Usage (Axiom P2)
        if ("/shared/" in file_path or "/models/" in file_path) and "BaseModel" not in content:
            self.log("Pydantic 미사용 발견", 2)
            results.append("❌ FAIL: Axiom P2 (Strict Schema) - 공용 데이터 모델은 Pydantic BaseModel을 사용하십시오.")

        # 3. Async/Await (Axiom P3)
        if ("main.py" in file_path or "/core/" in file_path) and "async def" not in content and "def " in content:
            self.log("Async IO 위반 의심", 1)
            results.append("⚠️ WARNING: Axiom P3 (Async IO) - MCP 코어 로직은 비동기(async def)를 권장합니다.")

        # 4. Naming Convention (Axiom P4)
        ...
        for name in bad_names:
            if f" {name} " in content or f"={name}" in content or f"{name}=" in content:
                ...
                results.append(f"❌ FAIL: Naming Convention - 축약어({name}) 대신 풀네임(view_controller, navigation)을 사용하십시오.")
                break

        # 5. Docstring Mandatory (Axiom P5)
        ...
        if len(functions) > len(docstrings) + 2: # 약간의 여유를 줌 (내부 함수 등 고려)
            ...
            results.append("💡 [Documentation] 주요 함수/클래스에 대한 Docstring 작성을 권장합니다. 📝")

        # 6. Axiom 5 (Mechanical Integrity)
