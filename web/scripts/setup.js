/**
 * [대장님 🎯] Operator Hub 통합 환경 자율 설치 스크립트 (Encapsulated Edition) 🛡️⚡️
 * 위치: web/scripts/setup.js
...
// [대장님 🎯] 현재 위치(web/scripts) 기준 두 단계 위가 진짜 프로젝트 루트입니다. 🕵️‍♂️⚡️
const rootDir = path.join(__dirname, '..', '..');
const webDir = path.join(rootDir, 'web');
const venvDir = path.join(rootDir, '.venv');
const isWindows = process.platform === 'win32';

console.log('\n🌟 [OPERATOR-GENESIS] 통합 지휘소 환경 구축을 시작합니다... 🛡️⚡️');
...
function ensureNode() {
  ...
  if (parseInt(currentVersion) < 18) {
    console.log(`⚠️ Node.js 버전이 낮습니다 (현재: ${process.version}). 설치를 시도합니다...`);
    if (isWindows) {
...
function ensurePython() {
  ...
  if (!tryCommand(`${pyCmd} --version`)) {
    console.log('⚠️ Python 3가 발견되지 않았습니다. 즉시 설치를 시도합니다...');
    if (isWindows) {
      if (tryCommand('winget --version')) {
...
try {
  ...
  console.log('\n⚡️ [3/3] 백엔드 규약 패키지(requirements.txt) 자동 설치...');
