...
/**
 * [대장님 🎯] 현재 지휘소(MCP)에 등록된 모든 전문 기술 유닛(Units) 리스트를 동적으로 스캔하여 반환합니다. 🛡️⚡️
 * 물리적 경로 정보를 포함하여 보안 및 이식성을 확보합니다.
 */
export async function GET() {
  try {
    const unitsPath = path.join(process.cwd(), '..', 'units');
    
    if (!fs.existsSync(unitsPath)) {
      return NextResponse.json({ units: [] });
    }
// 디렉토리 리스트를 스캔하여 유닛 이름을 추출합니다 (캐시 및 숨김 파일 제외) 🕵️‍♂️
